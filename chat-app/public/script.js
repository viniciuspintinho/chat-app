...
function setupShopUI() {
    ...
    if (profileArea && !document.getElementById('shop-btn')) {
        ...
        shopContainer.style = "display:flex; align-items:center; gap:10px; margin-left:10px; margin-top:5px;";
...
function updateCoins(amount) {
    ...
    if (balanceDisplay) balanceDisplay.innerText = `🪙 ${fofoCoins}`;
}
...
function sendMessage() {
    ...
    if (text) {
        ...
        if (text.startsWith('/love ')) {
            ...
            socket.emit('message', { type: 'system', content: `📢 ${userData.name} atualizou o letreiro!`, color: 'var(--accent)' });
        }
...
socket.on('message', (data) => {
    ...
    if (data.type === 'system') {
        ...
        const replyHTML = data.replyTo ? `<div class="msg-reply-info" style="background:rgba(255,255,255,0.05); border-left:3px solid var(--accent); padding:5px; margin-bottom:8px; border-radius:4px; font-size:0.7rem"><strong>${data.replyTo.name}:</strong> ${data.replyTo.text}...</div>` : '';
...
socket.on('updateUserList', (users) => {
    ...
    if(list) {
        list.innerHTML = users.map(u => {
            ...
            return `<div class="user-item" onclick="document.getElementById('message').value += '@${u.name} '">
